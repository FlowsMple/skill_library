...
- Mirror the upstream training transform's *spatial* image preprocessing (letterbox/square-padding, padding color, final input size) inside the deployment adapter — do not rely on the downstream HF image processor or `resize_images(...)` to "rescue" an aspect-mismatched input. Stretching a `(240, 320)` frame to `(224, 224)` looks like training to a casual reader but is a different visual distribution; the diffusion/flow-matching head collapses to ≈mean output and the arm twitches in place. See the "Image Spatial Alignment" section in `references/xpolicylab-policy-contract.md`.
- When upstream applies a non-trivial action normalization mode (`q99`, `min_max`, `mean_std`, `binary`, ...), invert with the **same statistics family** at deployment. `dataset_statistics.json` typically stores every stat (`min`/`max`/`mean`/`std`/`q01`/`q99`) so picking the wrong pair silently rescales the action and produces over- or under-extended joints. The saved `mask` field is also frequently generated from key names (e.g. "gripper" → False), not from the per-element check the trainer actually uses, and must not be trusted blindly. See the "Action Normalization Mode" section in `references/xpolicylab-policy-contract.md`.
- When the upstream model takes a multi-frame observation window (`obs_horizon > 1`, `observation_indices = [-K, ..., 0]`, `n_obs_steps`, `history_frames`, etc.), the deploy adapter must maintain a per-env per-view rolling image buffer and feed the same flat frame list at the same relative offsets the dataloader used. Single-frame `encode_obs` will silently send the wrong number of channels into the obs-merging linear (or split into wrong time chunks for video patchifiers) — depending on the architecture this either crashes at `state_dict` load with a shape mismatch when loading the pretrain ckpt, or runs through and collapses the diffusion head to ≈mean output (twitching in place). The released checkpoint's `obs_horizon` is the source of truth even when the training yaml claims a different value; verify by inspecting the ckpt's `obs_merger.weight` (or equivalent) before trusting either side. See the "Observation Window / History Frames" section in `references/xpolicylab-policy-contract.md`.
...
8. Add or adapt only the policy-local files needed for XPolicyLab: `model.py`, `deploy.py`, `deploy.yml`, `process_data.sh`, `train.sh`, `eval.sh`, `setup_eval_policy_server.sh`, `setup_eval_env_client.sh`, `install.sh`, `INSTALLATION.md`, and upstream-local helper modules.
...
- `XPolicyLab/policy/<PolicyName>/eval.sh`: orchestrator that allocates a free `policy_server_port`, launches `setup_eval_policy_server.sh` in the background, then runs `setup_eval_env_client.sh`, and cleans up the server on exit. Accepts 11 args: `dataset_name task_name ckpt_name env_cfg_type expert_data_num action_type seed policy_gpu_id env_gpu_id policy_conda_env eval_env_conda_env`. `task_name` is the simulator task; `ckpt_name` resolves the checkpoint and can differ (for example `cotrain`).
- `XPolicyLab/policy/<PolicyName>/setup_eval_policy_server.sh`: starts `setup_policy_server.py` inside the policy environment, bound to `policy_server_port` and `policy_server_host` (default `localhost` for cross-machine deployment). All policy-specific `--overrides` (checkpoint paths, model paths, attn impl, etc.) belong here.
- `XPolicyLab/policy/<PolicyName>/setup_eval_env_client.sh`: thin wrapper around `XPolicyLab/utils/setup_env_client.sh`, invoked inside `eval_env_conda_env`. Switching between `debug` / `sim` / `real` is controlled by `deploy.yml`'s `eval_env` field, not by editing this script.
